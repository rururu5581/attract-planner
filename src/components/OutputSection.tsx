...
const LoadingSpinner = () => (
    <div className="flex justify-center items-center">
      <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-morich-red"></div>
    </div>
);
...
const OutputSection: React.FC<OutputSectionProps> = ({ isLoading, error, script, rawScript }) => {
  const renderContent = () => {
      ...
      return (
          <div className="text-left py-4">
              <div className="flex items-center justify-center mb-6">
                 <LoadingSpinner />
                 <p className="ml-4 text-gray-600 font-medium">最高のアトラクトトークを生成中です...</p>
              </div>
    ...
    if (error) {
      return <div className="text-center py-10 text-red-600 bg-red-50 p-4 rounded-lg whitespace-pre-wrap">{`エラーが発生しました: ${error}`}</div>;
    }
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm w-full mt-8">
      <div className="flex justify-between items-center mb-6">
        ...
        {script.length > 0 && !isLoading && !error && (
